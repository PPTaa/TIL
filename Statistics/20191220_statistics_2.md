...
$$
E(a)=a\\
E(ax)=aE(X)\\E(X+Y)=E(X)+E(Y)\\E(X-Y)=E(X)-E(Y)
$$
분산의 성질
$$
V(a)=0\\V(aX)=a^2V(X)\\V(X+Y)=V(X)+V(Y)+2Cov(X,Y)\\V(X-Y)=V(X)+V(Y)-2Cov(X,Y)\\V(X)=\sum[X-E(X)]^2*P(X)=\sum X^2*P(X)-[E(X)]^2
$$
기댓값 계산
...
- 연속확률분포: (표준)정규분포, 지수분포, t-분포, F-분포, x^2분포